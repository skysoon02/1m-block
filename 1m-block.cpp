...
void read_sort(char *file_name){
	FILE* fp = fopen(file_name, "r");	//open
	if(fp==NULL){
	...
	while(!feof(fp)){	//read
        ...
        char *p = strtok(buf, ",");
        p = strtok(NULL, "\n");
        dict.push_back(string(p));
    }
	fclose(fp);	//close
	
	sort(dict.begin(), dict.end());	//sort
	printf("Read and Sort Complete!\n");
}


bool check(string host_name){
	return binary_search(dict.begin(), dict.end(), host_name);	
}
	      ...
	      struct nfq_data *nfa, void *data)
{
	...
	*/
	...
	char request[9][10]={"GET", "POST", "HEAD", "PUT", "DELETE", "CONNECT", "OPTIONS", "TRACE", "PATCH"};
	for(int i=0; i<9; i++){
		if(strncmp((char*)buf, request[i], strlen(request[i]))==0){
			http_check=1;
			break;
		}
	}
	...
	for(int i=0; i<size-4; i++){	//check host
		if(strncmp((char*)buf+i, "Host:", 5)==0){
			for(int j=i+6; j<size; j++){
				if(buf[j]==0x0d){
					char host_name[100] = "";
					strncpy(host_name, (char*)buf+i+6, j-(i+6));
					printf("\nhost_name: %skkkkkkk\n", host_name);
					if(check(string(host_name))==true){
						printf("DROP %s!!\n", host_name);
